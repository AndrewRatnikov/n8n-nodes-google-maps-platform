...
- **`scan-community-package` has no local-scan mode.** Confirmed 2026-08-14: `npx @n8n/scan-community-package <name>[@version]` always fetches the package from the npm registry (`GET registry.npmjs.org/<name>`) — there's no flag to point it at a local directory or tarball, and even `--help` gets treated as a package name and 404s. This means it can only be run *after* `npm publish` (step 7), not as a pre-publish gate the way "verification readiness" checklists imply. If it fails post-publish, the fix is a version bump and republish, not a local retry — budget for that possibility rather than assuming a clean first publish.
